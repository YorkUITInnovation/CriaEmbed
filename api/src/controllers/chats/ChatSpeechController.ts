...
export class ChatSpeechController extends BaseController {
  ...
  constructor(private embedService: EmbedService = new EmbedService()) {
    super();
  ...
  public async speech(
    ...
    try {
      ...
      const stream = await this.service.getChatSpeech(
        chatId,
        messageId,
        language
      );
      ...
      switch (e.constructor) {
        case ChatContentNotFoundError:
          ...
          return {
            ...
            code: "NOT_FOUND",
            message:
              "The chat exists but the cached chat content could not be found or is expired."
          };
        case CriaError:
