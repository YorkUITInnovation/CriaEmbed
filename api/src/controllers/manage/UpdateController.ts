...
export class UpdateController extends BaseController {
  ...
  public async update(
    ...
    @Header("X-Internal-Service") internalService?: string
  ): Promise<UpdateResponse> {
    try {
      const botConfig: IBotEmbed = await this.service.updateBot(
        { ...config, botName: botId },
        apiKey,
        // Criabot push must not sync back, or publish toggles oscillate.
        { syncToCriabot: internalService !== "criabot" }
      );
      this.setStatus(200);
      ...
      switch (e.constructor) {
        ...
        default:
          ...
          return {
            timestamp: Date.now().toString(),
            status: 500,
            message: `Internal error occurred! Class: '${
              e.constructor.name
