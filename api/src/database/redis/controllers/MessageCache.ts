...
export default class MessageCache extends RedisController {
  // Redis EX is in seconds - 6 hours.
  private EXPIRE_AFTER = 60 * 60 * 6;
  private generateId = () =>
    "message-" +
    Date.now().toString(36) +
    Math.random().toString(36).substring(2);
  private buildKey = (chatId: string, messageId: string) =>
    `cria-embed:chats:${chatId}:messages:${messageId}`;
  ...
  async get(chatId: string, messageId: string): Promise<string | null> {
    return this.redis.get(this.buildKey(chatId, messageId));
  }
}
