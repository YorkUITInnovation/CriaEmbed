...
export default class AudioCache extends RedisController {
  // Redis EX is in seconds - 30 minutes.
  private EXPIRE_AFTER = 60 * 30;
  private buildKey = (chatId: string, messageId: string) =>
    `cria-embed:chats:${chatId}:messages:${messageId}:audio`;
  ...
  async get(chatId: string, messageId: string): Promise<Buffer | null> {
    return this.redis.getBuffer(this.buildKey(chatId, messageId));
  }
}
