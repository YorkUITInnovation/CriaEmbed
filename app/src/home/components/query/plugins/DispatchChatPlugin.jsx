...
export function dispatchCommand(editor, command, text) {
  ...
  document.dispatchEvent(
    new CustomEvent("commandSend", { detail: { command, text } })
  );
}
...
const DispatchChatPlugin = () => {
  ...
  useEffect(() => {
    ...
    document.addEventListener("chatStreamComplete", setEditable);
    document.addEventListener("chatStreamError", setEditable);
    ...
    return () => {
      ...
      document.removeEventListener("chatStreamComplete", setEditable);
      document.removeEventListener("chatStreamError", setEditable);
